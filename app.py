...
def recognize_faces1(faces, frame, knn):
  save_first_image = True  # Flag to control saving the first image
  for (x, y, w, h) in faces:
    crop_img = frame[y:y + h, x:x + w, :]
    if faces.any() and save_first_image:
      # Get the user's Downloads folder path
      pass
      
      
      
       # Set flag to False after saving
...
def recognize_faces(faces, frame, knn):
  save_first_image = True  # Flag to control saving the first image
  for (x, y, w, h) in faces:
    crop_img = frame[y:y + h, x:x + w, :]
    if faces.any() and save_first_image:
      # Get the user's Downloads folder path
      downloads_path = os.path.expanduser("C:/Users/ganesh/Desktop/BlockBash/firstImg")
      save_path = os.path.join(downloads_path, "first_image_with_box.jpg")  # Create a filename
      cv2.imwrite(save_path, crop_img)
      save_first_image = False  # Set flag to False after saving
      recognize_faces1(faces, frame, knn)
    resized_img = cv2.resize(crop_img, (50, 50)).flatten().reshape(1, -1)
    output = knn.predict(resized_img)
    cv2.putText(frame, str(output[0]), (x, y - 15), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 1)
  return frame
...
def generate_frames():
    ...
    while True:
        ...
        if not success:
            ...
            frame, faces = detect_faces(frame, facedetect)
            
            frame = recognize_faces(faces, frame, knn)
...
def all_students():
    import requests
    url="https://newsapi.org/v2/everything?q=tesla&from=2024-03-22&sortBy=publishedAt&apiKey=5bfcb7b40245474ca2d799dc82f86447"
